...
const StickyMobileCTA = ({ onClick }: { onClick: () => void }) => (
  <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 z-50 md:hidden shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
    <button onClick={onClick} className="block w-full bg-slate-900 text-white text-center font-bold py-3 rounded-xl shadow-lg">
      Candidati Ora
    </button>
  </div>
);

type Page = 'home' | 'how-it-works' | 'benefits' | 'salaries' | 'testimonials';
  ...
  const handleNavClick = (page: Page, sectionId?: string) => {
    ...
    if (sectionId && page === 'home') {
      setTimeout(() => {
        ...
        // Special case: if we are on other pages and click 'join', go home then scroll
        setCurrentPage('home');
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-indigo-100 selection:text-indigo-900 pb-20 md:pb-0">
      ...
      {currentPage === 'how-it-works' ? (
        <HowItWorks onCtaClick={() => handleNavClick('home', 'join')} />
      ) : currentPage === 'benefits' ? (
        <Benefits onCtaClick={() => handleNavClick('home', 'join')} />
      ) : currentPage === 'salaries' ? (
        <Salaries onCtaClick={() => handleNavClick('home', 'join')} />
      ) : currentPage === 'testimonials' ? (
        <Testimonials onCtaClick={() => handleNavClick('home', 'join')} />
      ) : (
        <>
          {/* Hero Section */}
          <header className="relative pt-24 pb-24 px-6 overflow-hidden">
            ...
            <div className="max-w-6xl mx-auto text-center relative z-10">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
                <button onClick={() => handleNavClick('home', 'join')} className="w-full sm:w-auto px-10 py-5 bg-slate-900 text-white rounded-2xl font-bold text-lg hover:bg-slate-800 transition-all shadow-2xl shadow-slate-900/30 transform hover:-translate-y-1 flex items-center justify-center gap-3">
             ...
             <div className="max-w-6xl mx-auto px-6 relative z-10">
                <div className="grid md:grid-cols-2 gap-16 items-center">
                   <div>
                      ...
                      <button onClick={() => handleNavClick('salaries')} className="text-white border-b border-white/30 pb-1 hover:text-blue-400 hover:border-blue-400 transition-colors font-medium">
          ...
          <section id="process" className="py-20 bg-white">
            <div className="max-w-6xl mx-auto px-6">
               ...
               <div className="text-center">
                  <button onClick={() => handleNavClick('how-it-works')} className="text-blue-600 font-bold hover:underline">Scopri i dettagli del processo &rarr;</button>
               </div>
            </div>
          </section>
          ...
          <section id="benefits" className="py-24 bg-slate-50">
            ...
            <BentoGrid>
              ...
              <Card 
                ...
                className="md:col-span-3 cursor-pointer hover:border-blue-300 transition-colors"
                icon={<TrendingUpIcon className="w-6 h-6 text-green-600" />}
              >
                 <div onClick={() => handleNavClick('salaries')} className="mt-4 h-32 relative flex items-end justify-between gap-2 px-2 border-b border-slate-100 pb-1">
                    {/* Data Bars */}
          ...
          {/* Reviews Section (Preview for Home) */}
          <section id="reviews" className="py-20 bg-white border-b border-slate-100">
             <div className="max-w-4xl mx-auto px-6">
                <div className="flex flex-col items-center text-center mb-12">
                   ...
                   <button onClick={() => handleNavClick('testimonials')} className="mt-10 text-blue-600 font-bold hover:underline">
                      Vedi tutte le storie di successo &rarr;
                   </button>
                </div>
             </div>
          </section>
      ...
      {/* Form Section - Dark Theme (Always visible or just for home? Let's keep it always visible but maybe change title on HowItWorks) */}
      ...
      <StickyMobileCTA onClick={() => handleNavClick('home', 'join')} />
    </div>
  );
};
